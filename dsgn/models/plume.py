...
class PLUMENet(nn.Module):
    def __init__(self, cfg=None):
        ...
        self.feature_extraction = plume_feature_extraction(cfg)
    ...
    def forward(self, left, right, calibs_fu, calibs_baseline, calibs_Proj, calibs_Proj_R=None):
        ...
        if self.cfg.RPN3D_ENABLE:
            ...
            rpn_img_feature =  F.grid_sample(refimg_fea.unsqueeze(2), grid, align_corners=True)   
            ...
            # (64, 190, 20, 300)
            Voxel = self.rpn3d_conv(rpn_voxel_feature) # (N, 64, 190, 20, 300)
            ...
            Voxel = self.rpn3d_pool(Voxel) # (N, 64, 190, 5, 300)
            Voxel = Voxel.permute(0, 1, 3, 2, 4).reshape(N, -1, self.GRID_SIZE[0], self.GRID_SIZE[2]).contiguous() # (N, 64 * 5, 190, 300) 

            Voxel_BEV = self.rpn3d_conv2(Voxel) #(N, 64 * 2, 190, 300)
